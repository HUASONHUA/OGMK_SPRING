...
<%
    ...
    try {
        String protocol = request.getProtocol().toLowerCase().substring(0, 
        		request.getProtocol().indexOf("/"));
        String ipAddress = java.net.InetAddress.getLocalHost().getHostAddress();
        String url = "https://" + ipAddress + ":" + request.getLocalPort() + request.getContextPath() + "/member/credit_card_back.do";    
        ...
        obj.setItemName("OGKM信用卡-" + order.getId() + " $" 
        + order.getTotalAmountWithFee() + "元"); //請將非常好書改成你的web site 中文/英文名稱
        ...
        String orderIdStr = "OGKM" //請改成你的webapp名稱,產生一個allpay要求的訂單編號字串(必須<20個英數字字元,且不得與之前的訂單重複)
